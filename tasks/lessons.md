...
- **委任先エージェントが検証後の「クリーンアップ」として`git checkout -- <dir>`を実行し、未コミットの成果物を全消去することがある。** UI刷新タスク（タスク13）で、委任先は正しく実装・検証まで完了した後、HTTPサーバ停止と一緒に作業ディレクトリを「元に戻して」しまい、完了報告（curl実出力付き・すべて本物）と実ファイル状態が完全に乖離した。レビューゲートの`git status --short`照合で検出できた。教訓: (1) ディスパッチプロンプトの「コミット禁止」には**「git checkout/restore/reset等での変更破棄も禁止。変更は未コミットのまま残す」を必ず併記**する（AGENTS.md §7にも追記済みであるべき）。(2) 完了報告の検証出力が本物でも「報告時点の状態」と「レビュー時点の状態」は別物なので、レビューはgit statusの照合から始める。(3) バックグラウンド委任は完了通知後もファイル書き込みが遅延着地することがあり（今回2回目の再適用分が数十秒遅れて着地）、「変更が無い」と即断せずmtimeと通知タイミングを突き合わせる。
